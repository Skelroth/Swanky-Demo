<?php
   ...
   class Repeatable extends KUserDefinedField{
      ...
      function tag_handler( $params, $node ){
         ...
         foreach( $children as $child ){
            if( $child->type==K_NODE_TYPE_CODE && strtolower($child->name)=='editable' ){
               ...
               $attr = $FUNCS->get_named_vars( array('type'=>''), $child_params );
         ...
         <div class="tableholder">
                     ...
                     <?php
                        if( is_null($this->rendered_data) ){ // not handling posted data
                           ...
                           for( $y=0; $y<count($this->cells); $y++ ){
                              ...
                              $html .= '<span class="shim">'. $c->get_data() .'</span>';
            ...
            <?php
               if( count($arr_deleted_html) ){
                  ...
                  $html .= '<pre>' . htmlspecialchars( $str_deleted_html, ENT_QUOTES ) . '</pre>';
                  $html .= '</div>';
                  echo $html;
               }
            ?>
      ...
      function store_posted_changes( $post_val ){
         ...
         if( count($data) ){
            if( strlen(trim($_POST['_f_'.$this->name.'_sortorder'])) ){
               $arr_sort = array_map( "trim", explode( ',', $_POST['_f_'.$this->name.'_sortorder'] ) );
               $tmp = array(); $x = 0;
         ...
         for( $row=0; $row<count($data); $row++ ){
            ...
            for( $y=0; $y<count($this->cells); $y++ ){
               ...
               /*if( $c->k_type == 'thumbnail' || $c->k_type == 'hidden' ||  $c->k_type == 'message' || $c->k_type == 'group' ){
                  unset( $c );
                  continue;
               }*/
               if( $c->k_type== 'checkbox' ){
                  $separator = ( $c->k_separator ) ? $c->k_separator : '|';
                  $sep = '';
                  $str_val = '';
                  if( isset($data[$row][$c->name]) ){
                     foreach( $data[$row][$c->name] as $v ){
                        $str_val .= $sep . $v;
                        $sep = $separator;
                     }	
                  }
                  $c->store_posted_changes( $str_val );
               }
               else{
